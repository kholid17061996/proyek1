...
export default function PengajarLayout({
  ...
  const navigation = [
    ...
    { name: 'Target & Pencapaian Siswa', href: '/pengajar/setoran', icon: BookOpen },
  ]
      ...
      `}>
        <div className="flex items-center justify-between h-20 px-6 border-b border-white/10 bg-white/5 min-w-[18rem]">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-teal-300 to-teal-600 rounded-xl flex items-center justify-center text-slate shadow-lg shadow-teal-500/30">
              <BookOpen size={24} />
            </div>
            <span className="text-xl font-bold text-white tracking-wide">Panel Pengajar</span>
          </div>
          <button 
      ...
      <div className="flex-1 flex flex-col h-screen min-w-0 overflow-hidden relative z-10 transition-all duration-500">
        ...
        <header className="h-20 shrink-0 bg-white/10 backdrop-blur-xl border-b border-white/20 shadow-sm flex items-center justify-between px-6 lg:px-10 z-30">
          <div className="flex items-center gap-4">
            <button 
              className={`text-white hover:text-teal-300 transition-colors ${sidebarOpen ? 'lg:hidden' : 'block'}`}
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:block text-right">
              <p className="text-sm font-bold text-white">Ustaz / Ustazah</p>
